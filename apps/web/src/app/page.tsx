  ...
  return <div className="landing-page">
    ...
    <main id="main-content">
      <section className="landing-hero">
        <div className="landing-hero__copy"><span className="landing-kicker"><span /><Sparkles size={15} />AI meeting intelligence для команд</span><h1>Из разговора —<br /><em>в понятный результат.</em></h1><p>HATTAMA превращает запись совещания в структурированный протокол, транскрипт и список поручений. На русском, қазақша и в смешанной речи.</p><div className="landing-hero__actions"><Link href="/register" className="button button--primary button--lg">Создать протокол<ArrowRight size={18} /></Link><a href="#workflow" className="button button--secondary button--lg"><Play size={17} fill="currentColor" />Посмотреть как работает</a></div><div className="landing-proof"><span><CheckCircle2 size={16} />Локальное распознавание речи</span><span><CheckCircle2 size={16} />Анализ текста через GPT API</span></div></div>
      ...
      <section id="security" className="security-section"><div className="security-mark"><LockKeyhole size={34} /></div><div><span className="section-number">03 / БЕЗОПАСНОСТЬ</span><h2>Аудио обрабатывается локально.</h2><p>Аудио распознаётся локально. Текст транскрипта отправляется в GPT API для анализа и подготовки протокола.</p></div><ul><li><Check size={17} />HttpOnly-сессии</li><li><Check size={17} />Вход через аккаунт</li><li><Check size={17} />Локальное хранение</li></ul></section>
