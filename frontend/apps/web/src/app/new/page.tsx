...
import { createMeetingSchema, type CreateMeetingInput } from "@repo/contracts";
...
export default function NewMeetingPage() {
  ...
  const { register, handleSubmit, setValue, control, formState: { errors } } = useForm<CreateMeetingInput>({
  ...
  const onSubmit = (values: CreateMeetingInput) => {
    setStep(0);
    mutation.mutate(values, {
      onSuccess: () => { toast.success("Протокол сформирован", { description: "6 поручений готовы к проверке" }); router.push("/meetings/operational-review"); },
